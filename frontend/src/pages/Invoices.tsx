...
const Invoices: React.FC = () => {
    ...
    const submitInvoice = async (submitData: InvoiceSubmitPayload) => {
      ...
      if (editingInvoice) {
        ...
        if (emailNotification?.attempted && emailNotification?.sent) {
          notifySuccess('Invoice updated successfully. Update email delivered to recipient.');
        } else if (emailNotification?.attempted && !emailNotification?.sent) {
          notifyWarning(`Invoice updated, but update email failed: ${emailNotification.reason || 'Unknown error'}`);
        } else {
          notifyWarning(`Invoice updated, but email was not attempted: ${emailNotification?.reason || 'No recipient email'}`);
        }
        ...
        if (invoiceStatus === 'sent') {
          if (emailNotification?.attempted && emailNotification?.sent) {
            notifySuccess('Invoice sent successfully. Email delivered to recipient.');
          } else if (emailNotification?.attempted && !emailNotification?.sent) {
            notifyWarning(`Invoice marked as sent, but email failed: ${emailNotification.reason || 'Unknown error'}`);
          } else {
            notifyWarning(`Invoice marked as sent, but email was not attempted: ${emailNotification?.reason || 'No recipient email'}`);
          }
        } else {
  ...
  const handleConfirmSendFromPreview = async () => {
    ...
    try {
      ...
      const message = getErrorMessage(error, 'Failed to save and send invoice. Please try again.');
      setError(message);
      notifyError(message);
    } finally {
    ...
    <div>
                    ...
                    <div className="mt-3">
                      ...
                      {formData.invoiceType === 'other' && (
                        <input
                          ...
                          placeholder="Custom type (max 10)"
                          maxLength={10}
      ...
      {showSendPreview && previewData && (() => {
        ...
        const payLines = companyPayInstructions
          ...
          : companyPhone ? [`Mobile Money (MoMo): ${companyPhone}`, `Bank Transfer: Use reference ${previewData.invoiceNumber}`] : [];
        ...
        const invoiceLabel = invoiceTypeLabels[typeKey] || String(previewData.invoiceType || 'INVOICE').toUpperCase();
            ...
            <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl my-4">
              ...
              <div className="flex items-center justify-between border-b border-gray-200 px-6 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">This is how the PDF will appear when sent</p>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6 text-sm font-sans">
                  ...
                  <div className="flex justify-between items-start mb-5">
                    <div>
                      ...
                      <p className="font-bold text-gray-900">{previewClient?.name || 'Client'}</p>
                  ...
                  <div className="flex justify-end mb-4">
                    <div className="w-48 space-y-1 text-xs">
                      ...
                      {previewData.taxApplied && previewTax > 0 && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Sales Tax ({previewData.taxRate || 0}%)</span>
                  ...
                  {payLines.length > 0 && (
                    <div className="border-t border-gray-100 pt-3">
                      <p className="text-xs font-semibold mb-1" style={{ color: accentColor }}>How to Pay</p>
              ...
              <div className="border-t border-gray-200 px-6 py-4 flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={sendingFromPreview}
                >
                  {sendingFromPreview ? 'Sending...' : 'Save and Send'}
                </button>
              </div>
            </div>
          </div>
