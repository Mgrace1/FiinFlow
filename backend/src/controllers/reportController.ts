...
export const generateExpensesPDF = async (req: AuthRequest, res: Response) =>{
  try {
    ...
    const generatedBy = currentUser?.name || currentUser?.email || 'N/A';
    ...
    const drawHeader = (isFirstPage: boolean) => {
      ...
      doc.text('Report date', 360, 232, { width: 95 });
      doc.text('Generated by', 360, 254, { width: 95 });
  ...
  try {
    ...
    const drawHeader = (compact = false, title = 'FINANCIAL SUMMARY REPORT') => {
      if (compact) {
        ...
        doc.text(`Generated by: ${generatedBy}`, 220, 68);
